// УДАЛИТЬ ЭТУ СТРОКУ: import { db, messaging, saveFcmToken, removeFcmToken, getJudges, updateJudgeDevice, createJudgeKey, deleteJudgeKey } from '/apb-admin/firebase-init.js';
    // УДАЛИТЬ ЭТУ СТРОКУ: import { getToken } from 'firebase/messaging';

    // Остальной код settings.js без изменений, но используя глобальные React и lucide
    // (ПРЕДПОЛАГАЕТСЯ, ЧТО firebase-init.js и lucide уже загружены и доступны)
    ...
    // Компонент Settings
    window.Settings = function({ show, onClose, adminDeviceId }) { // Делаем компонент глобальным
        React.useEffect(() => { injectCriticalStyles(); }, []);

        // ... (остальной код компонента Settings без изменений) ...
        // ВСЕ ссылки на React, lucide, db, messaging должны работать как глобальные переменные
        // Например, вызовы React.useState, React.useEffect, lucide.createIcons, db, messaging, saveFcmToken и т.д.

        // --- Остальной код функции Settings (скопируйте его из оригинального файла, убедившись, что используются глобальные переменные) ---
        ...
        const loadJudges = async () => {
            ...
            try {
                ...
                if (cached && cachedTime && (now - parseInt(cachedTime)) < 5 * 60 * 1000) {
                    ...
                    const judgesList = await getJudges(); // Глобальная функция из firebase-init
        ...
        const handleDeviceToggle = async (judgeKey, currentDeviceId, checked) => {
            if (!checked) {
                try {
                    await updateJudgeDevice(judgeKey, null); // Глобальная функция
                    setJudges(prev => prev.map(j => j.key === judgeKey ? { ...j, deviceId: null } : j));
                    setToast({ show: true, message: 'Устройство отвязано' });
                    setTimeout(() => setToast({ show: false, message: '' }), 1500);
                } catch (err) {
                    console.error('Ошибка отвязки:', err);                    setToast({ show: true, message: 'Ошибка отвязки' });
                    setTimeout(() => setToast({ show: false, message: '' }), 2000);
                }
            } else {
        ...
        const handleDeleteKey = async (judge) => {
            try {
                 await deleteJudgeKey(judge.key); // Глобальная функция
        ...
        const handleGenerateKey = async () => {
            setGenerating(true);
            try {
                await createJudgeKey(); // Глобальная функция
                await loadJudges();
                ...
                if (pushEnabled) {
                    try {
                        ...
                        if (Notification.permission === 'granted') {
                            ...
                            if (token) await saveFcmToken(adminDeviceId, token, navigator.userAgent); // Глобальная функция
                        }
                    } catch (err) {
                    ...
                    try { await removeFcmToken(adminDeviceId); } catch (err) { console.error(err); } // Глобальная функция
                }
            };
            handlePushToggle();
            ...
            React.createElement(
                ...
                React.createElement('div', { className: 'space-y-4' },
                    React.createElement('div', { className: 'flex items-center justify-between' },
                        ...
                        React.createElement('label', { className: 'relative inline-flex items-center cursor-pointer' },
                            React.createElement('input', {
                                type: 'checkbox', className: 'sr-only peer', checked: pushEnabled,                                onChange: e => { const v = e.target.checked; setPushEnabled(v); localStorage.setItem('pushEnabled', v); }
                            }),
                    ...
                    React.createElement('p', { className: 'text-[10px] text-gray-400 uppercase tracking-wider' },
                        'Включите, чтобы получать у новых оценках даже когда приложение закрыто.'
                    )
                ),
                ...
                React.createElement('div', { className: 'space-y-4' },
                                            ...
                                            React.createElement('td', { className: 'px-4 py-3' },
                                                React.createElement('div', { className: 'flex items-center gap-3' },
                                                    React.createElement('label', { className: 'relative inline-flex items-center cursor-pointer', title: 'Отвязать устройство' },
                                                        React.createElement('input', {                                                            type: 'checkbox', className: 'sr-only peer', checked: !!judge.deviceId,
                    ...
                    React.createElement('div', { className: 'flex justify-end mt-4' },
                        React.createElement('button', {
                            ...
                            className: 'bg-black text-white px-4 py-2 rounded-full text-[1 font-medium uppercase tracking-wider hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-2 '
                        },
                            generating
