    ...
    reader.onload = async (e) => {
      try {
        ...
        for (const row of rows) {
          const number = row['Номер'] || row['номер'] || row['№'] || '';
          const firstName = row['Имя'] || row['имя'] || '';
          const lastName = row['Фамилия'] || row['фамилия'] || '';
          const city = row['Город'] || row['город'] || '';
          const discipline = row['Номинация'] || row['номинация'] || '';
          let date = row['Дата'] || row['дата'];
          if (!number || !firstName || !lastName || !city || !discipline) {
            console.warn('Пропущена строка из-за неполных данных', row);
            continue;
          }
          let dateObj = null;
          if (date) {
          ...
          const q = query(
            ...
            where('date', '==', dateObj ? new Date(dateObj) : null)
          );
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('textarea', {
          className: 'w-full p-3 border border-gray-200 rounded-2xl text-[12px] leading-relaxed focus:outline-none focus:border-black',
          rows: '6',
          value: systemPrompt,
          onChange: (e) => setSystemPrompt(e.target.value),
          placeholder: 'Системный промпт для YandexGPT...'
        }),
        React.createElement(
          'div',
          ...
          React.createElement(
            'button',
            {
              ...
              className: 'bg-black text-white px-4 py-2 rounded-full text-[10px] font-medium uppercase tracking-wider hover:bg-gray-800'
            },
            'Сохранить промпт'
          )
        )
      ...
      // Управление участниками и архив
      React.createElement(
        'div',
        { className: 'space-y-4' },
        ...
        React.createElement(
          ...
          React.createElement(
            'label',
            {
              className: `bg-black text-white px-4 py-2 rounded-full text-[10px] font-medium uppercase tracking-wider hover:bg-gray-800 cursor-pointer text-center ${importing ? 'opacity-50' : ''}`,
            },
            React.createElement('input', {
              type: 'file',
              accept: '.xlsx, .xls',
          ...
          React.createElement(
            'button',
            {
              ...
              className: `bg-black text-white px-4 py-2 rounded-full text-[10px] font-medium uppercase tracking-wider hover:bg-gray-800 ${exportingArchive ? 'opacity-50' : ''}`
            },
            exportingArchive ? 'Архивация...' : 'Архив (экспорт всех данных)'
          )
        ),
        React.createElement(
          'p',
          { className: 'text-[9px] text-gray-400 uppercase tracking-wider' },
          'Excel файл должен содержать колонки: Номер, Имя, Фамилия, Город, Номинация, Дата (YYYY-MM-DD)'
        )
      )
    ),
